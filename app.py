...
class RegisterWindow:
    ...
    def register(self):
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "CALL sp_dang_ky_tai_khoan(%s, %s, NULL);",
                (username, password)
            )
            result = cur.fetchone()
            conn.commit()
            ...
            messagebox.showinfo("Thành công", f"Đăng ký tài khoản thành công. Mã tài khoản: {user_id}", parent=self.top)
            self.top.destroy()

        except Exception as e:
            if conn:
                conn.rollback()
            messagebox.showerror("Lỗi", friendly_error_message(e), parent=self.top)
        finally:
            if cur:
                cur.close()
            if conn:
    ...
    def login(self):
        ...
        try:
            ...
            if row:
                ...
                root = tk.Tk()
                app = LibraryApp(root, user_id, username, vai_tro)
                root.mainloop()
            else:
...
class LibraryApp:
    ...
    def borrow_books(self):
        ...
        try:
            ...
            cur.execute(
                ...
                (self.user_id, int(reader_id), due_date, copy_list, note)
            )
            result = cur.fetchone()
            conn.commit()
    ...
    def pay_fine(self):
        ...
        try:
            conn = get_connection()
            cur = conn.cursor()
            cur.execute("CALL sp_thanh_toan_tien_phat(%s, %s, NULL);", (self.user_id, int(borrow_id)))
            result = cur.fetchone()
            conn.commit()

            total = result[0] if result else 0
            messagebox.showinfo("Thành công", f"Thanh toán tiền phạt thành công.\nTổng tiền: {format_money(total)} VND")
    ...
    def export_overdue_csv(self):
        try:
            ...
            with open(path, "w", newline="", encoding="utf-8-sig") as f:
                writer = csv.writer(f)
                writer.writerow([
                    "PhieuMuonID", "BanDocID", "HoTen", "Email", "SDT",
                    "NgayMuon", "NgayHenTra", "SoNgayQuaHan", "TrangThai"
                ])
                writer.writerows(rows)
